...
import styles from './ComboRadioSelectBox.module.scss';
...
const ComboRadioSelectBox = ({
  ...
  const getLabel = (optionLabel) => {
    if (translations.choiceTemplate) {
      return optionLabel;
    }

    if (translations.translatePlaceholders) {
      return translatePlaceholders(optionLabel);
    }
  ...
  const getIcon = (icon) => {
    if (highlightOnSelect) {
      return (
        <div
          className={`${styles['logo-highlight']} icon-highlight`}
          style={{
            mask: `url(${icon.srcSet}) no-repeat center center`,
            WebkitMask: `url(${icon.srcSet}) no-repeat center center`,
            WebkitMaskSize: 'contain',
          }}
        />
      );
    }
    return <Image className="mx-auto image-icon img-fluid" {...icon} />;
  };
  ...
  return (
    <div className="row" id={fieldName}>
      ...
      <div className="col-lg">
        <div className="row">
          {singleChoices.map((option, index) => (
            <div
              key={`${fieldName}-${option.value}`}
              className={`d-flex ${gridColumns[buttonsPerRow]} ${
                (index + 1) % buttonsPerRow !== 0 ? 'pr-0' : ''
              }`}
            >
              <div className="form-check mb-2 d-flex flex-fill mb-lg-2">
                {option.jumpTo ? (
                  <a
        ...
        {selectOptions && selectOptions.length > 0 && (
          <select
            ...
            onChange={handleChange}
          >
            {sortChoices(selectOptions, sortOrder, t).map((option) => (
              <option
                key={`${fieldName}-${option.value}}`}
                value={option.value}
